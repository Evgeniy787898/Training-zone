...
Документ описывает публичные REST-эндпоинты, которые использует Telegram WebApp и внешние интеграции. Все запросы выполняются к `https://<your-host>/api/*` и требуют передачи заголовка `X-Telegram-Id` со значением `telegram_id` пользователя (или `X-Profile-Id` с UUID профиля).
...
### `GET /api/sessions/week?date=2024-03-31`
...
Возвращает ответ движка без участия Telegram.

```json
{
  "message": "Сохрани: сделать заминку перед сном",
  "mode": "chat",
  "persist": true
}
```
...
```json
{
  "reply": "🗒️ Зафиксировал: «сделать заминку перед сном». Если нужно напомнить — напиши время.",
  "intent": "note_save",
  "confidence": 0.86,
  "candidates": [
...
Ошибки: `422 validation_failed` (пустое сообщение).

### `GET /api/assistant/notes?limit=20`
  ...
  "notes": [
    {
      ...
      "content": "Сделать фото прогресса",
      "tags": ["контроль"],
      "source": "chat",
...
Создаёт заметку в журнале.

```json
{
  "content": "Добавить растяжку в среду",
  "title": "Среда",
  "tags": ["разминка", "идея"]
}
```
...
Ошибки: `422 validation_failed` (если `content` пуст).

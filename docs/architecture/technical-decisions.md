...
## Обзор платформы
- **Язык и фреймворк бота**: Node.ts 18+, библиотека `telegraf` для взаимодействия с Telegram API.
- **Встроенный движок**: ответы и планы формируются локально на основе `services/internalAssistantEngine.ts`.
...
6. **Assistant API**
   - REST-эндпоинты `POST /api/assistant/reply` и `GET|POST /api/assistant/notes` позволяют использовать движок вне Telegram: получать ответы с учетом истории диалога, читать и создавать заметки.
   - Ответы API используют те же сервисы истории (`services/history.ts`) и правила внутреннего движка, поэтому чат и WebApp остаются синхронизированы.
...
- **Пайплайн**:
  ...
  3. Серверная функция строит график (например, через `chart.ts` headless) и сохраняет изображение в Supabase Storage (папка `analytics/{profile_id}/{slug}/{date}`) вместе с JSON-данными.
...
1. **Локальная подготовка**:
   ...
   - `npm install` в `backend/src/bot`, `supabase functions secrets set` для Edge Functions.

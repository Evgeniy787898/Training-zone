...
- Под капотом работает связка Supabase (хранилище данных через Prisma), встроенный движок тренера (`internalAssistantEngine.ts`) и Vue 3 WebApp для визуализации.
...
- Таблица `assistant_notes` хранит личные заметки пользователя, которые можно создавать из чата или через API `/api/assistant/notes`.

...
- В боте подключён rule-based NLU (`backend/src/services/nlu.ts`), распознающий ключевые intents: просмотр плана, отчёт, статистика, перенос, напоминание, мотивация и режим восстановления.
- Свободный текст в `backend/src/bot/index.ts` направляется в соответствующие сценарии: запуск /plan, запуск отчёта, вызов подробной статистики, запуск редакторов настроек и сценария переноса.
- Перенос тренировки реализован в `backend/src/bot/commands/reschedule.ts`: пользователь выбирает дату из нескольких вариантов или переносит «на завтра» одной фразой.
...
- `services/history.ts` унифицирует хранение истории между ботом и HTTP API, чтобы WebApp мог запрашивать / сохранять ответы без рассинхронизации.

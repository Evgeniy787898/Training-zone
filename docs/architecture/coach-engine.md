...
```
backend/src/services/internalAssistantEngine.ts
...
- `backend/src/services/conversation.ts` — маршрутизирует ответы тренера, подсказывает синхронизацию плана при отсутствии расписания.
- `backend/src/services/planner.ts` — экспортирует API для бота и WebApp (`generateTrainingPlan`, `analyzeTrainingReport`).
- `backend/src/services/aiCommandRouter.ts` — приводит запросы к структуре `interpretCommand()`.
- `backend/src/services/supabase.ts` (DatabaseService) — слой доступа к данным через Prisma (профиль, сессии, шаблоны `ai_templates`).
...
- Для `plan_today` и `plan_week` используются ближайшие сессии из базы; если расписание пустое, бот просит обновить план в WebApp.
...
- Выходит в ответ на запросы «справка по триггерам», «что ты можешь», а также доступна через WebApp API `POST /api/assistant/reply`.

## Генерация планов

`generateTrainingPlan()`:

1. Определяет частоту тренировок (по найденным сессиям или настройкам профиля).
2. Загружает актуальные тренировки из `training_sessions` за ближайшую неделю и формирует текстовое резюме.
3. Прикладывает историю последних сессий, если она передана в контекст.
4. Возвращает объект вида `{ rawText, structured }`, где `structured.plan` содержит данные из БД либо `null`, если расписание пустое.
...
1. **Новые интенты** — добавить паттерны в `backend/src/services/nlu.ts`, затем обработку в `interpretCommand()` и соответствующий блок в `generateTrainerReply()` или `generateGeneralReply()`.
...
5. **Память и заметки** — доработать `buildNoteSaveReply()` (категории, атачменты), расширить API `/api/assistant/notes`, добавлять напоминания поверх сохранённых заметок.

## Логирование и кеши

- Срезы плана из БД кешируются в `dialog_state` с ключом `ui_cached_plan` и TTL до конца недели.

...
- **Навигация**: нижняя панель с вкладками — «Сегодня», «План недели», «Отчёт», «Прогресс», «Упражнения», «Справка», «Настройки». Реализовано на Vue 3 с Vue Router.
...
2. Приложение вызывает `GET /api/sessions/today` и показывает карточку тренировки.
3. Кнопка «Отметить выполненной» отправляет `PUT /api/sessions/{id}` с `status = done`. При успехе отображается toast и данные обновляются.
4. Кнопка «Перенести» показывает подсказку вернуться в чат и написать соответствующую команду.

### Просмотр плана на неделю
1. Экран «План недели» запрашивает `GET /api/sessions/week` (с датой недели).
...
1. Экран «Прогресс» одновременно запрашивает:
   - `GET /api/reports/volume_trend?range=30d`
   - `GET /api/reports/rpe_distribution?range=30d`
   - `GET /api/achievements`
2. Диаграммы строятся с помощью Recharts: линейный график объёма и столбчатая диаграмма распределения RPE.
3. Статистика отображает регулярность (из `/api/profile`), средний объём и долю тяжёлых тренировок.
4. Список достижений показывает до 10 последних записей.

### Каталог упражнений
1. Экран «Упражнения» запрашивает `GET /api/exercises/catalog` и отображает карточки прогрессий.
2. При выборе карточки загружается история `GET /api/exercises/{exerciseKey}/history` и показываются уровни.
...
### Настройки
1. Значения по умолчанию берутся из `/api/profile` (`notificationTime`, `timezone`, `notificationsPaused`).
2. Изменения отправляются запросом `PATCH /api/profile`.
...
- Базовый URL считывается из `VITE_API_BASE_URL` (например, `http://localhost:3001` - порт backend V2).

// TZONA V2 - Local Responder Service
// Ported from V1 localResponder.js
...
function buildLocalReply({ message, profile, history }: any) {
    if (!message) {
        return null;
    }
...
function buildThanksReply(profile: any) {
...
function buildGreetingReply({ profile, history, message }: any) {
...
function buildCapabilityReply(profile: any) {
    const frequency = resolveFrequency(profile);
    const introVariants = [
...
function buildTriggersCheatsheet() {
    ...
    return intro ? [intro, '', ...list, '', outro].join('\n') : [list.join('\n'), '', outro].join('\n');
}

function topicReply(snippet: any, profile: any) {
    const frequency = resolveFrequency(profile);
    const header = snippet.summary;
    const bullets = snippet.bullets.map((line: string) => `- ${line}`);
...
function determineGreetingTone({ message, profile, history }: any) {
    if (!message) {
        return 'neutral';
    }
    ...
    const lastAssistant = [...(history || [])].reverse().find((item: any) => item.role === 'assistant');
...
function resolveFrequency(profile: any): string | null {
    const value =
...
function extractLastPlanFocus(assistantMessage: any) {

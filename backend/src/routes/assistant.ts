...
type LatencyAlertSeverity = 'warn' | 'error';
type LatencyAlertReason = 'slow_turn' | 'slow_ratio' | 'average_latency';

interface LatencyAlertPayload {
...
const resolveSlowThreshold = (stats: any): number => {
    if (!stats) {
        return 3500;
    }
    const value = Number((stats as any).slow_threshold_ms);
...
const toLatencyValue = (value: unknown): number | null => {
...
const buildLatencyAlertMessage = (reason: LatencyAlertReason, context: { latencyMs: number | null; thresholdMs: number; ratio: number; samples: number }): string => {
...
const createLatencyAlert = (
    ...
    const threshold = Number.isFinite(stats?.slow_threshold_ms) ? Number(stats.slow_threshold_ms) : 3500;
...
const normalizeAlertEntry = (entry: any): LatencyAlertPayload | null => {
    if (!entry || typeof entry !== 'object') {
        return null;
    }
    const severity = entry.severity === 'error' ? 'error' : entry.severity === 'warn' ? 'warn' : null;
    const reason: LatencyAlertReason = ['slow_turn', 'slow_ratio', 'average_latency'].includes(entry.reason)
        ? entry.reason
        : 'slow_turn';
    const threshold = Number.isFinite(entry.threshold_ms) ? Math.round(Number(entry.threshold_ms)) : 3500;
    const latencyMs = toLatencyValue(entry.latency_ms ?? null);
    const average = toLatencyValue(entry.average_ms ?? null);
    const ratio = clampRatio(entry.slow_ratio ?? 0);
    const samples = toPositiveInteger(entry.samples ?? 0);
    const triggeredAt = typeof entry.triggered_at === 'string' ? entry.triggered_at : new Date().toISOString();
...
const normalizeAlertHistoryList = (value: any): LatencyAlertPayload[] => {

...
export class SessionRepository {
    ...
    async deriveStructuredSessionData(notesPayload: StructuredNotes | null, profileId: string) {
        ...
        if (Array.isArray(notesPayload.results)) {
            ...
            const exerciseKeysToFetch = new Set<string>();
            const levelLookupsNeeded = new Map<string, Set<string>>();
            ...
            for (const entry of entries) {
                ...
                if (!explicitExerciseId) {
                    exerciseKeysToFetch.add(exerciseKey);
                }
                ...
                if (!explicitLevelId && levelCode) {
                    const collection = levelLookupsNeeded.get(exerciseKey);
                    if (collection) {
                        collection.add(levelCode);
                    } else {
                        levelLookupsNeeded.set(exerciseKey, new Set([levelCode]));
                    }
                }
            }

            const exerciseLookup = new Map<string, string>();
            if (exerciseKeysToFetch.size > 0) {
                const exercises = await this.prisma.exercise.findMany({
                    where: { exerciseKey: { in: Array.from(exerciseKeysToFetch) } },
            ...
            >();
            if (levelLookupsNeeded.size > 0) {
                const orConditions = Array.from(levelLookupsNeeded.entries()).flatMap(([exerciseKey, levels]) =>
            ...
            for (const entry of entries) {
                ...
                let exerciseId = explicitExerciseId ?? exerciseLookup.get(exerciseKey) ?? null;

                let exerciseLevelId = explicitLevelId ?? null;
                if (!exerciseLevelId && levelCode) {
                    const metadata = levelLookup.get(`${exerciseKey}:${levelCode}`);
                    if (metadata) {
                        exerciseLevelId = metadata.id;
                        if (targetSets === null && metadata.sets !== null) {
                            targetSets = metadata.sets;
                        }
                        if (targetReps === null && metadata.reps !== null) {
                            targetReps = metadata.reps;
                        }
                    }
                }
